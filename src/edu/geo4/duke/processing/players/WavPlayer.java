...
public class WavPlayer implements ICaller {
  ...
  public WavPlayer(File file, ICallee operator) {
    this(operator);
    try {
      myAudioInputStream = AudioSystem.getAudioInputStream(file);
  ...
  public WavPlayer(AudioInputStream ais, ICallee operator) {
    this(operator);
    myAudioInputStream = ais;
  }

  private WavPlayer(ICallee operator) {
    myOperator = operator;
  ...
  private static float[] byteToFloat(byte[] bytes, int bytesPerSample) {
    ...
    for (int i = 0; i < output.length; i++) {
      ...
      long composite =
          (long) ((frames[7] << 56) | (frames[6] << 48) | frames[5] << 40 | frames[4] << 32
              | frames[3] << 24 | frames[2] << 16 | frames[1] << 8 | (frames[0] & 0xFF));
  ...
  /**
   * Returns the bytes from a Wav format byte stream corresponding to the specified channel number.
   * 
   * @param sampleSizeInBits Number of bits in each sample of the input stream
   * @param numChannels Total number of channels present in the input stream data
   * @param outChannelNum Desired output channel (0-indexed)
   * @return The byte array of just a single channel from a wav file with multiple channels.
   */
  ...
  private static byte[] interleaveChannels(ArrayList<byte[]> channels, int sampleSizeInBits) {
    ...
    for (byte[] chan : channels) {
      if (chan.length != first) {
        throw new RuntimeException("Channel lengths were different");
      }
    }
